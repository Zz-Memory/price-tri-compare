...
export async function fetchHomeRecommendations({
  ...
  if (isVercel) {
    // 完全复刻 mock/homeRecommendations.js 的逻辑，确保语义一致
    const { Random } = Mock;
    ...
    function generateList(count = 12, p = 'jd', c = 'phone') {
      return Array.from({ length: count }, () => genItem(p, c));
    }

    const list = generateList(pageSize, platform || 'jd', category || 'phone');
    const mockResp = {
      code: 0,
      message: 'ok',
      data: { list, page, pageSize, hasMore: page < 5 }
    };
    return mockResp.data.list;
  }
